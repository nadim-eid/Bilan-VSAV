...
const MEDICAMENT_OPTIONS = [
  { value: 'hypertension', label: 'Traitement hypertension' },
  { value: 'diabete', label: 'Traitement diabète' },
  { value: 'anticoagulant', label: 'Anticoagulant' },
  { value: 'antiagregant', label: 'Antiagrégant plaquettaire' },
  { value: 'cardiaque', label: 'Traitement cardiaque' },
  { value: 'anxiolytique_antidepresseur', label: 'Anxiolytique / antidépresseur' },
...
const EVENEMENT_OPTIONS = [
  ...
  { value: 'exposition_chaleur_froid', label: 'Exposition chaleur/froid' },
...
const RISQUE_OPTIONS = [
  ...
  { value: 'chirurgie_recente', label: 'Chirurgie récente' },
  { value: 'grossesse_post_partum', label: 'Grossesse/post-partum' },
  { value: 'contraception_hormonale', label: 'Contraception hormonale' },
  { value: 'cancer_connu', label: 'Cancer connu' },
  { value: 'anticoagulant', label: 'Traitement anticoagulant' },
...
// Chaque symptôme cliqué déclenche directement un nouveau PQRST, titré avec le symptôme.
const SYMPTOME_OPTIONS = [
  ...
  { value: 'fourmillements', label: 'Fourmillements / engourdissements' },
  { value: 'perte_sensibilite', label: 'Perte de sensibilité' },
  { value: 'trouble_parole', label: 'Trouble de la parole' },
  { value: 'faiblesse_membre', label: "Sensation de faiblesse d'un membre" },
            ...
            <div className="flex flex-col gap-3 items-stretch">
              ...
              {form.BRULURE.brulure === 'oui' && (
                <div className="flex flex-col gap-4 pt-2 border-t border-neutral-800">
                  <div className="flex flex-col gap-1.5">
                    <span className="text-xs text-neutral-500 uppercase tracking-wide">
                      Refroidissement — durée réglable
                    </span>
                    <div className="flex items-center gap-2">
                      <InputBox
      ...
      return (
        <>
          <div className="flex flex-col gap-2">
            ...
            <span className="text-xs text-neutral-600 italic">
              Chaque symptôme sélectionné ouvre son propre PQRST.
            </span>
            <div className="flex flex-wrap gap-2">
              {SYMPTOME_OPTIONS.map((opt) => (
                <button
                  key={opt.value}
                  onClick={() => addPqrst(opt.label)}
            ...
            <div className="flex items-center gap-2">
              <InputBox
                value={autreSymptome}
                onChange={setAutreSymptome}
                placeholder="Autre symptôme…"
                width="w-full"
              />
              <button
                onClick={() => {
                  const label = autreSymptome.trim();
                  if (!label) return;
                  addPqrst(label);
                  setAutreSymptome('');
                }}
                className="flex items-center gap-1.5 text-xs font-semibold uppercase tracking-wide px-3 py-2 rounded-md shrink-0"
                style={{ backgroundColor: ACCENT, color: '#fff' }}
              >
          ...
          <SamplerField
            ...
            choiceOptions={MEDICAMENT_OPTIONS}
            choiceValue={form.SAMPLER.sampler_m_choices}
            onChoiceChange={(v) => updateField('SAMPLER', 'sampler_m_choices', v)}
            choiceMulti
          />
          <SamplerField
            letter="P"
          ...
          <SamplerField
            ...
            choiceOptions={EVENEMENT_OPTIONS}
            choiceValue={form.SAMPLER.sampler_e_choices}
            onChoiceChange={(v) => updateField('SAMPLER', 'sampler_e_choices', v)}
            choiceMulti
          />
          <SamplerField
            letter="R"
            ...
            choiceOptions={RISQUE_OPTIONS}
            choiceValue={form.SAMPLER.sampler_r_choices}
            onChoiceChange={(v) => updateField('SAMPLER', 'sampler_r_choices', v)}
            choiceMulti
          />
        </>
      );
