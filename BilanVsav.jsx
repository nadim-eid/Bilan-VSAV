...
const PAGE_FIELDS = {
  ...
  A: ['obstruction', 'liberation_effectuee', 'trauma', 'collier_pose'],
  B: [
    'fr',
    'fr_ample',
    'fr_reguliere',
...
function formatSurveillanceReading(reading) {
  return [
    ...
    reading.conscience && `Consciente : ${reading.conscience === 'oui' ? 'Oui' : 'Non'}`,
    reading.eva !== '' && reading.eva !== undefined && reading.eva !== null && reading.eva !== '' && `EVA ${reading.eva}/10`,
  ]
    .filter(Boolean)
    .join(' — ');
}
...
function formatSurveillanceDelta(reading, previous) {
  ...
  add('EVA', signedDelta(reading.eva, previous.eva));
  return parts.join(' · ');
}
...
function computeMissingChecks(data) {
  ...
  const aPairs = [
    ['liberté des voies aériennes', data.A?.obstruction, 'A_obstruction'],
...
function buildCompactSms(form, patientNum, recordedAt = Date.now()) {
  ...
  lines.push(...buildRcpLines(form.RCP, false));
...
export default function BilanVsav() {
  ...
  function addSurveillanceReading() {
    ...
    const hasValue = ['fr', 'fc', 'spo2', 'pa_sys', 'pa_dia', 'temperature', 'glycemie', 'conscience', 'eva'].some(
      (field) => !!draft[field]
    );
    if (!hasValue) {
  ...
  function commitRcpEvents(events) {
    const nextForm = { ...form, RCP: { events } };
            ...
            <>
                  ...
                  onChange={(v) => {
                    ...
                    setForm((f) => {
                      const shouldUpdateDebit =
                        (!f.B.o2_debit || f.B.o2_debit_auto) && !!O2_INTERFACE_DEFAULT_DEBIT[v];
                      return {
                        ...f,
                        B: {
                          ...f.B,
                          o2_interface: v,
                          ...(shouldUpdateDebit
                            ? { o2_debit: O2_INTERFACE_DEFAULT_DEBIT[v], o2_debit_auto: true }
                            : {}),
                        },
                      };
                    });
              ...
              <FieldCard id="B_o2_debit" label="Débit" filled={!!form.B.o2_debit}>
                <InputBox
                  ...
                  onChange={(v) => {
                    setForm((f) => ({ ...f, B: { ...f.B, o2_debit: v, o2_debit_auto: false } }));
                    if (bilanStartRef.current === null) bilanStartRef.current = Date.now();
                    if (saved) setSaved(false);
                  }}
          ...
          <div className="bg-neutral-900 border border-neutral-800 rounded-lg p-4 flex flex-col gap-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              ...
              <div>
                ...
                <ToggleGroup value={surveillanceDraft.conscience} onChange={(v) => updateSurveillanceDraft('conscience', v)} options={OUI_NON} />
              </div>
              <div>
                <span className="text-xs text-neutral-500">EVA</span>
...
function buildRcpLines(rcp, includeHistory = true) {
  ...
  if (includeHistory) {
    ...
    allEvents.forEach((event) => {
      ...
      lines.push(`  ${rcpEventTime(event, true)} — ${label}${elapsed ? ` (${elapsed})` : ''}${cancelled}`);
    });
  }
  lines.push('');
  return lines;
