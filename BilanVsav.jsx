...
function isHyperthermieDetected(form) {
  ...
  const signs = form.E.victime_env_signes || [];
  if (signs.length > 0) return true;
  if (isPastThreshold(form.E.temperature, (n) => n >= 39)) return true;
  if (isPastThreshold(form.B.fr, (n) => n > 30)) return true;
  if (isPastThreshold(form.B.spo2_air, (n) => n < 94)) return true;
  if (isPastThreshold(form.C.fc, (n) => n > 100)) return true;
  if (isPastThreshold(form.C.pa_gauche_sys, (n) => n < 90)) return true;
  if (isPastThreshold(form.C.pa_droite_sys, (n) => n < 90)) return true;
  return false;
}
...
function CoolingModal({ active, onStop }) {
  if (!active) return null;
  return (
    <div
      className="fixed inset-0 flex items-center justify-center z-50 px-4"
    ...
    >
      ...
      >
        <AlertTriangle size={40} style={{ color: AMBER }} />
        <h3 className="text-lg font-bold" style={{ fontFamily: "'Barlow Condensed', sans-serif" }}>
          Fin des 20 minutes — arrêt du refroidissement
        </h3>
        <p className="text-sm text-neutral-400">
...
export default function BilanVsav() {
  ...
  const coolingTimer = useCountdown(20 * 60);
            ...
            <div className="flex flex-col gap-4 pl-1 border-l-2 border-neutral-800">
              <div className="flex flex-col gap-1.5 pl-3">
                ...
                <MultiToggleGroup
                  ...
                  onChange={(v) => updateField('E', 'victime_env_signes', v)}
              ...
              {form.BRULURE.brulure === 'oui' && (
                <div className="flex flex-col gap-4 pt-2 border-t border-neutral-800">
                  <div className="flex flex-col gap-1.5">
                    ...
                    <TimerBox timer={coolingTimer} label="Démarrer 20 min" />
                  </div>
                  <div className="flex flex-col gap-1.5">
  ...
  return (
    <div className="h-screen w-full flex flex-col bg-neutral-950 text-neutral-100" style={{ fontFamily: "'Inter', sans-serif" }}>
      ...
      <CoolingModal active={coolingTimer.done} onStop={coolingTimer.reset} />
