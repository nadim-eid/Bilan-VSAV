...
const PAGE_TITLES = {
  ...
  SURVEILLANCE: 'Surveillance — Relevés successifs',
...
function reconcileSamplerCrossLinks(sampler, changedField, nextValues) {
  ...
  const resolvedAutoP = uniqueValues(autoP);
  ...
  const resolvedAutoR = uniqueValues([...autoRFromM, ...autoRFromP]);
...
function formatSurveillanceReading(reading) {
  return [
    ...
    reading.glycemie && `Gly ${reading.glycemie} ${reading.glycemie_unit || 'g/L'}`,
  ]
    .filter(Boolean)
    .join(' — ');
}
...
function computeAlertSummary(data) {
  ...
  add('A', 'Voies aériennes', [data.A.obstruction === 'non' && 'liberté des voies aériennes : non']);
...
function computeMissingInfo(data) {
  const items = [];
  const add = (message) => {
    if (message && !items.includes(message)) items.push(message);
  };
  const hasReal = (arr) => (arr || []).some((v) => v !== 'aucun');
  const missingLabels = (pairs) => pairs.filter(([, value]) => !value).map(([label]) => label);

  if (!data.TYPE?.categorie) add('Catégorie de victime non renseignée');

  const xMissing = missingLabels([
    ['traumatisme', data.X?.trauma],
    ['hémorragie', data.X?.hemorragie],
  ]);
  if (xMissing.length) add(`X non évalué : ${xMissing.join(', ')}`);

  const aMissing = missingLabels([
    ['liberté des voies aériennes', data.A?.obstruction],
    ['libération effectuée', data.A?.liberation_effectuee],
  ]);
  if (aMissing.length) add(`A non évalué : ${aMissing.join(', ')}`);

  const bMissing = missingLabels([
    ['FR', data.B?.fr],
    ['amplitude', data.B?.fr_ample],
    ['régularité', data.B?.fr_reguliere],
    ['SpO2 sous air', data.B?.spo2_air],
  ]);
  if (!(data.B?.fr_signes || []).length) bMissing.push('signes associés');
  if (bMissing.length) add(`B incomplet : ${bMissing.join(', ')}`);

  const hasCompletePa =
    (!!data.C?.pa_gauche_sys && !!data.C?.pa_gauche_dia) ||
    (!!data.C?.pa_droite_sys && !!data.C?.pa_droite_dia);
  const cMissing = missingLabels([
    ['FC', data.C?.fc],
    ['pouls symétrique', data.C?.pouls_sym],
    ['pouls bien frappé', data.C?.pouls_frappe],
    ['TRC', data.C?.trc],
  ]);
  if (!hasCompletePa) cMissing.push('au moins une PA complète');
  if (!(data.C?.signes || []).length) cMissing.push('signes associés');
  if (cMissing.length) add(`C incomplet : ${cMissing.join(', ')}`);

  const dMissing = missingLabels([
    ['PCI', data.D?.pci],
    ['état de conscience', data.D?.etat],
    ['orientation', data.D?.orientation],
    ['pupilles', data.D?.pupilles],
    ['sensibilité/motricité mains', data.D?.sens_mains],
    ['sensibilité/motricité pieds', data.D?.sens_pieds],
  ]);
  if (!(data.D?.neuro_signes || []).length) dMissing.push('signes associés');
  if (dMissing.length) add(`D incomplet : ${dMissing.join(', ')}`);

  const eMissing = missingLabels([
    ['température', data.E?.temperature],
    ['lésion cachée', data.E?.lesion],
    ['victime coincée/comprimée', data.E?.coince],
    ['amputation', data.E?.amputation],
  ]);
  if (eMissing.length) add(`E incomplet : ${eMissing.join(', ')}`);

  if (!data.BRULURE?.brulure) add('Évaluation de brûlure non renseignée');

  if (data.B.o2_active === 'oui') {
    if (!data.B.o2_interface) add('Interface O2 à renseigner');
    if (!data.B.o2_start_time) add('Heure de début O2 à renseigner');
    if (!data.B.o2_bottle_size) add('Taille de bouteille O2 à renseigner');
    if (!data.B.o2_pressure) add('Pression de la bouteille O2 à renseigner');
    if (!data.B.o2_debit) add('Débit O2 à renseigner');
    if (!data.B.spo2_o2) add('SpO2 sous O2 non renseignée');
    const threshold = o2BottleThreshold(data.B.o2_bottle_size);
    const pressure = Number(String(data.B.o2_pressure).replace(',', '.'));
    if (threshold !== null && Number.isFinite(pressure) && pressure <= threshold) {
      add(`Pression O2 au seuil de réserve ou en dessous (${threshold} bars)`);
    }
  }
  (data.B?.o2_sessions || []).forEach((session, index) => {
    ...
    if (missing.length) add(`Épisode O2 ${index + 1} incomplet : ${missing.join(', ')}`);
    if (session.start_time && !isValidTime(session.start_time)) add(`Épisode O2 ${index + 1} : heure de début invalide`);
    if (session.end_time && !isValidTime(session.end_time)) add(`Épisode O2 ${index + 1} : heure de fin invalide`);
  });

  if (data.X.garrot_pose === 'oui' && !data.X.garrot_heure) {
    add('Heure de pose du garrot manquante');
  }
  if (data.E.amputation === 'oui' && data.E.amputation_garrot === 'oui' && !data.E.amputation_garrot_heure) {
    add('Heure de pose du garrot (amputation) manquante');
  }

  if (data.X.hemorragie === 'oui' && (!data.X.hemorragie_sites || data.X.hemorragie_sites.length === 0)) {
    add("Localisation de l'hémorragie non renseignée");
  }

  if (data.E.amputation === 'oui' && !data.E.amputation_localisation) {
    add("Localisation de l'amputation à renseigner");
  }

  if (data.E.amputation_segment_retrouve === 'oui' && !data.E.amputation_conditionnement) {
    add('Conditionnement du segment non renseigné');
  }
  ...
  if (fastPositif && !data.FAST.temps && !data.FAST.temps_choice) {
    add("Heure d'apparition des signes FAST à renseigner");
  }
  const fastIndicated =
    hasReal(data.D?.neuro_signes) ||
    data.D?.sens_mains === 'non' ||
    data.D?.sens_pieds === 'non' ||
    (data.SAMPLER?.symptom_choices || []).some((v) =>
      ['trouble_parole', 'faiblesse_membre', 'troubles_visuels'].includes(v)
    );
  if (fastIndicated && (!data.FAST?.face || !data.FAST?.arm || !data.FAST?.speech)) {
    add('FAST incomplet alors que des signes neurologiques sont renseignés');
  }

  if (hasReal(data.D.neuro_signes) && !data.D.neuro_signes_depuis_heure) {
    add("Heure d'apparition des signes neurologiques manquante");
  }
  if (hasReal(data.B.fr_signes) && !data.B.fr_signes_heure) {
    add("Heure d'apparition des signes respiratoires manquante");
  }
  if (hasReal(data.C.signes) && !data.C.signes_heure) {
    add("Heure d'apparition des signes circulatoires manquante");
  }

  if (data.D.pci === 'oui' && !data.D.pci_duree) {
    add('Durée de la PCI non renseignée');
  }
  if (data.D.pc_repete === 'oui' && !data.D.pc_nombre) {
    add('Nombre de pertes de connaissance non renseigné');
  }

  if (data.BRULURE.brulure === 'oui') {
    if (!data.BRULURE.brulure_etendue) {
      add('Étendue de la brûlure non renseignée');
    }
    const hasLoc =
      (data.BRULURE.brulure_loc_choices && data.BRULURE.brulure_loc_choices.length > 0) || !!data.BRULURE.brulure_loc;
    if (!hasLoc) {
      add('Localisation de la brûlure non renseignée');
    }
  }
  ...
  if (!data.D.glycemie) {
    ...
    const hasDiabetesContext =
      ...
      (data.SAMPLER?.sampler_m_choices || []).some((value) => ['antidiabetique', 'insuline'].includes(value));
    ...
    if (reasons.length) {
      add(`Glycémie non renseignée — contrôle à envisager (${reasons.join(', ')}).`);
    }
  }

  const timeFields = [
    ['Heure de pose du garrot', data.X?.garrot_heure],
    ["Heure d'apparition respiratoire", data.B?.fr_signes_heure],
    ['Heure de début O2', data.B?.o2_start_time],
    ["Heure d'apparition circulatoire", data.C?.signes_heure],
    ["Heure d'apparition neurologique", data.D?.neuro_signes_depuis_heure],
    ['Heure de début de compression', data.E?.coince_depuis],
    ['Heure du garrot d’amputation', data.E?.amputation_garrot_heure],
    ["Heure d'apparition FAST", data.FAST?.temps],
    ['Heure de dernière prise orale', data.SAMPLER?.sampler_l_time],
    ["Heure de l'événement", data.SAMPLER?.sampler_e_time],
  ];
  timeFields.forEach(([label, value]) => {
    if (value && !isValidTime(value)) add(`${label} invalide (format attendu : HH:MM)`);
  });

  const numericChecks = [
    ['FR', data.B?.fr, 1, 100],
    ['SpO2 sous air', data.B?.spo2_air, 1, 100],
    ['SpO2 sous O2', data.B?.spo2_o2, 1, 100],
    ['Pression O2', data.B?.o2_pressure, 1, 300],
    ['Débit O2', data.B?.o2_debit, 0.1, 100],
    ['FC', data.C?.fc, 1, 300],
    ['PA gauche systolique', data.C?.pa_gauche_sys, 1, 300],
    ['PA gauche diastolique', data.C?.pa_gauche_dia, 1, 200],
    ['PA droite systolique', data.C?.pa_droite_sys, 1, 300],
    ['PA droite diastolique', data.C?.pa_droite_dia, 1, 200],
    ['Température', data.E?.temperature, 15, 45],
    ['Étendue de brûlure', data.BRULURE?.brulure_etendue, 0, 100],
  ];
  numericChecks.forEach(([label, value, min, max]) => {
    if (value && !isNumberInRange(value, min, max)) add(`${label} : valeur invalide`);
  });
  if (data.D?.glycemie && !isPositiveNumber(data.D.glycemie)) add('Glycémie : valeur invalide');
  ...
  [
    ...
    if (Number.isFinite(sys) && Number.isFinite(dia) && dia >= sys) {
      add(`PA ${side} incohérente : la diastolique doit être inférieure à la systolique`);
    }
  });

  (data.SURVEILLANCE?.releves || []).forEach((reading, index) => {
    const prefix = `Surveillance relevé ${index + 1}`;
    if (!isValidTime(reading.heure)) add(`${prefix} : heure invalide`);
    const readingChecks = [
      [reading.fr, 1, 100],
      [reading.fc, 1, 300],
      [reading.spo2, 1, 100],
    ...
    if (readingChecks.some(([value, min, max]) => value && !isNumberInRange(value, min, max))) {
      add(`${prefix} : une constante est invalide`);
    }
    if (reading.pa_sys && reading.pa_dia) {
      ...
      if (Number.isFinite(sys) && Number.isFinite(dia) && dia >= sys) {
        add(`${prefix} : PA incohérente (diastolique ≥ systolique)`);
      }
    }
    if (reading.glycemie && !isPositiveNumber(reading.glycemie)) add(`${prefix} : glycémie invalide`);
  });
  (data.SAMPLER?.pqrst_list || []).forEach((entry, index) => {
    if (entry.t_heure && !isValidTime(entry.t_heure)) add(`PQRST ${index + 1} : heure de début invalide`);
    if (entry.t_duree && !isPositiveNumber(entry.t_duree)) add(`PQRST ${index + 1} : durée invalide`);
  });
  ...
  [
    ...
    if ((values || []).includes('aucun') && (values || []).some((value) => value !== 'aucun')) {
      add(`${page} incohérent : « Aucun » est sélectionné avec d'autres ${label}`);
    }
  });
  if (
    ...
    add('SAMPLER incohérent : détails d’allergie présents sans allergie confirmée');
  }
...
function missingItemPage(message) {
  ...
  if (
    /^B |respiratoire|SpO2|Interface O2|Heure de début O2|Taille de bouteille O2|Pression O2|Débit O2|Épisode O2/i.test(
      message
    )
  )
    return 'B';
  ...
  if (/Surveillance/i.test(message)) return 'SURVEILLANCE';
  return null;
}

function getMissingPages(data) {
  return uniqueValues(computeMissingInfo(data).map(missingItemPage).filter(Boolean));
}
...
function buildRecapLines(data) {
  ...
  lines.push(PAGE_TITLES.SURVEILLANCE);
  if (surveillanceReadings.length === 0) {
    lines.push('  Aucun relevé successif');
  } else {
  ...
  if (automationAudit.length > 0) {
    lines.push('AUTOMATISATIONS APPLIQUÉES');
...
function buildCompactSms(form, patientNum) {
  const lines = [`BILAN EQUIPIER N°${patientNum} - ${new Date().toLocaleString('fr-FR')}`];
...
function sendBilanBySms(form, patientNum) {
  const text = buildCompactSms(form, patientNum);
    ...
    <div className="flex flex-col gap-5">
        ...
        >
          ...
          <ul className="flex flex-col gap-1 pl-1">
            {missingInfo.map((item, i) => {
              const page = missingItemPage(item);
              const content = (
                <>
          ...
          {onNavigate && (
            <button
              type="button"
              onClick={() => onNavigate(missingItemPage(missingInfo[0]), true)}
      ...
      <div className="flex flex-col gap-2">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-base font-bold uppercase tracking-wide" style={{ fontFamily: "'Barlow Condensed', sans-serif" }}>
            Surveillance — Relevés successifs
          </h3>
        </div>
        {(data.SURVEILLANCE?.releves || []).length === 0 ? (
          <p className="text-sm text-neutral-600 italic px-1">Aucun relevé de surveillance ajouté.</p>
        ) : (
...
export default function BilanVsav() {
  ...
  }, [form, patientNum, step, saved, draftChecked, draftCandidate]);
  ...
  function resumeDraft() {
    ...
    setStep(Math.max(0, Math.min(Number(draftCandidate.step) || 0, STEPS.length - 1)));
  ...
  function prefillSurveillanceDraft() {
    ...
    setSurveillanceDraft({
      ...
      glycemie_unit: latest?.glycemie_unit || form.D.glycemie_unit || 'g/L',
    });
  ...
  function navigateToMissing(page, enableReviewMode = false) {
    const targetIndex = STEPS.indexOf(page);
    if (targetIndex < 0) return;
    setReviewMissingOnly(enableReviewMode);
    setStep(targetIndex);
  }
  ...
  function goNext() {
    if (reviewMissingOnly) {
      const missingIndexes = getMissingPages(form)
        ...
        .sort((a, b) => a - b);
      const nextMissing = missingIndexes.find((index) => index > step);
      if (nextMissing !== undefined) {
        setStep(nextMissing);
      } else {
        setStep(STEPS.indexOf('RECAP'));
        setReviewMissingOnly(false);
      }
      return;
    }
  ...
  function renderStepContent() {
    ...
    if (s === 'A')
      return (
        <>
          ...
          <FieldCard label="Libération effectuée" filled={!!form.A.liberation_effectuee}>
            <ToggleGroup
    ...
    if (s === 'B')
      return (
        <>
          ...
          {form.B.o2_active === 'oui' && (
            <>
              <FieldCard label="Interface O2" filled={!!form.B.o2_interface}>
                <ToggleGroup
                  value={form.B.o2_interface}
                  onChange={(v) => updateField('B', 'o2_interface', v)}
                  options={O2_INTERFACE_OPTIONS}
                />
              </FieldCard>

              <FieldCard label="Heure de début O2" filled={!!form.B.o2_start_time}>
            ...
            <div className="flex flex-col gap-2">
              <div>
                ...
                <ToggleGroup
                  ...
                  onChange={(v) => updateField('D', 'glycemie_unit', v || 'g/L')}
    ...
    if (s === 'SAMPLER') {
      ...
      const traumaPrompt = (form.SAMPLER.sampler_e_choices || []).some((v) => ['traumatisme', 'chute_traumatisme', 'accident_circulation', 'agression'].includes(v));
      const autoRisks = [];
      ...
      return (
        <div className="flex flex-col gap-3">
          <p className="text-sm text-neutral-400 leading-relaxed">
            Ajoute chaque nouveau relevé avec son heure. Les constantes initiales du bilan restent inchangées.
          </p>
          <button
            type="button"
          ...
          <div className="bg-neutral-900 border border-neutral-800 rounded-lg p-4 flex flex-col gap-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <ToggleGroup value={surveillanceDraft.glycemie_unit} onChange={(v) => updateSurveillanceDraft('glycemie_unit', v || 'g/L')} options={GLYCEMIE_UNIT_OPTIONS} />
          ...
          <div className="flex flex-col gap-2">
            ...
            {readings.length === 0 ? (
              ...
              readings.map((reading, index) => {
                ...
                const elapsed = surveillanceElapsedLabel(reading, previous);
                return (
                  <div key={reading.id || `${reading.heure}_${index}`} className="bg-neutral-900 border border-neutral-800 rounded-md px-3 py-2.5 flex gap-3 items-start">
      ...
      <main ref={mainRef} className="flex-1 overflow-y-auto px-4 py-4">
        {reviewMissingOnly && (
          <div className="mb-3 flex items-center justify-between gap-3 rounded-md border px-3 py-2" style={{ borderColor: AMBER, backgroundColor: '#1A1508' }}>
            ...
            <button
              type="button"
              onClick={() => setReviewMissingOnly(false)}
      ...
      >
        {STEPS[step] !== 'RECAP' ? (
          <>
            ...
            >
              {reviewMissingOnly ? 'Point suivant' : 'Suivant'} <ChevronRight size={16} />
            </button>
          </>
        ) : (
          ...
          >
            {!viewing ? (
              ...
              <>
                ...
                <div className="mb-4">
                  <ExportButtons form={viewing.data} patientNum={viewing.patientNum} />
                </div>
