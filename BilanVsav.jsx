...
const ALLERGY_OPTIONS = [
  ...
  { value: 'insecte', label: "Piqûre / insecte" },
...
const ANTECEDENTS_OPTIONS = [
  ...
  { value: 'neurologique', label: 'Autre neurologique' },
  { value: 'asthme', label: 'Asthme' },
  { value: 'bpco', label: 'BPCO' },
  { value: 'respiratoire', label: 'Autre respiratoire' },
...
const MEDICAMENT_OPTIONS = [
  ...
  { value: 'psychotrope', label: 'Anxiolytique / antidépresseur / psychotrope' },
...
const EVENEMENT_OPTIONS = [
  ...
  { value: 'repas', label: 'Repas' },
  { value: 'exposition_chaleur', label: 'Exposition chaleur' },
  { value: 'exposition_froid', label: 'Exposition froid' },
...
const RISQUE_OPTIONS = [
  ...
  { value: 'immobilisation_recente', label: 'Immobilisation récente' },
  { value: 'chirurgie_recente', label: 'Chirurgie récente' },
  { value: 'hospitalisation_recente', label: 'Hospitalisation récente' },
...
const PAGE_FIELDS = {
  ...
  A: ['obstruction'],
  B: ['fr', 'fr_ample', 'fr_reguliere', 'fr_signes', 'spo2_air', 'spo2_o2', 'o2_debit'],
  ...
  E: ['temperature', 'victime_env', 'victime_env_signes', 'lesion', 'coince', 'coince_depuis', 'coince_zone_choices', 'coince_zone'],
...
export default function BilanVsav() {
  ...
  const coolingMinutes = parseFloat(String(form.BRULURE.cooling_duration_min).replace(',', '.')) || 20;
    ...
    if (s === 'D')
      return (
        <>
          ...
          <FieldCard
            label="Depuis"
            filled={!!form.D.neuro_signes_depuis_choice || !!form.D.neuro_signes_depuis_heure}
          >
            <div className="flex flex-col gap-2">
              <ToggleGroup
                value={form.D.neuro_signes_depuis_choice}
                onChange={(v) => updateField('D', 'neuro_signes_depuis_choice', v)}
                options={DEPUIS_DUREE_OPTIONS}
              />
              <InputBox
                ...
                <div className="flex flex-col gap-4 pt-2 border-t border-neutral-800">
                  <div className="flex flex-col gap-2">
                    ...
                    {form.BRULURE.cooling_done === 'oui' && (
                      <div className="flex flex-col gap-2">
                        <span className="text-xs text-neutral-500 uppercase tracking-wide">Durée du refroidissement</span>
                        <div className="flex items-center gap-2">
                          <InputBox
                            ...
                            width="w-24"
                          />
                          {coolingTimer.running && <span className="text-xs text-neutral-600 italic">en cours</span>}
                        </div>
                        {!!form.BRULURE.cooling_duration_min && <TimerBox timer={coolingTimer} label={`Démarrer ${Math.round(coolingMinutes)} min`} />}
                      </div>
                    )}
                  </div>
                  <div className="flex flex-col gap-1.5">
    ...
    if (s === 'SAMPLER') {
      ...
      return (
        <>
            ...
            <div key={entry.id} className="bg-neutral-900 border border-neutral-800 rounded-lg p-4 flex flex-col gap-4">
              ...
              <div className="flex flex-col gap-3 border-t border-neutral-800 pt-3">
                ...
                <div className="flex flex-wrap gap-3 items-end">
                  <div><span className="text-xs text-neutral-500">Heure de début</span><InputBox value={entry.t_heure || ''} onChange={(v) => updatePqrstField(entry.id, 't_heure', formatTimeInput(v))} placeholder="hh:mm" numeric width="w-24" /></div>
          ...
          <div className="bg-neutral-900 border border-neutral-800 rounded-lg p-4 flex flex-col gap-3">
            <div className="flex items-center gap-2"><span className="w-7 h-7 rounded flex items-center justify-center text-white text-sm font-bold" style={{ backgroundColor: ACCENT }}>A</span><span className="text-sm font-semibold uppercase">Allergies</span></div>
            ...
            {form.SAMPLER.allergy_status === 'oui' && <>
              <MultiToggleGroup value={form.SAMPLER.sampler_a_choices} onChange={(v) => updateField('SAMPLER', 'sampler_a_choices', v)} options={ALLERGY_OPTIONS} />
              ...
              <span className="text-xs text-neutral-500 uppercase">Réaction connue</span>
          ...
          <SamplerField letter="M" label="Médicaments / traitements" value={form.SAMPLER.sampler_m} onChange={(v) => updateField('SAMPLER', 'sampler_m', v)} choiceOptions={MEDICAMENT_OPTIONS} choiceValue={form.SAMPLER.sampler_m_choices} onChoiceChange={(v) => updateField('SAMPLER', 'sampler_m_choices', withExclusiveNone(form.SAMPLER.sampler_m_choices || [], v, 'aucun'))} choiceMulti />
          <div className="bg-neutral-900 border border-neutral-800 rounded-lg p-4 flex flex-col gap-2">
          ...
          <SamplerField letter="P" label="Passé médical" value={form.SAMPLER.sampler_p} onChange={(v) => updateField('SAMPLER', 'sampler_p', v)} choiceOptions={ANTECEDENTS_OPTIONS} choiceValue={form.SAMPLER.sampler_p_choices} onChoiceChange={(v) => updateField('SAMPLER', 'sampler_p_choices', v)} choiceMulti />

          <div className="bg-neutral-900 border border-neutral-800 rounded-lg p-4 flex flex-col gap-3">
            <div className="flex items-center gap-2"><span className="w-7 h-7 rounded flex items-center justify-center text-white text-sm font-bold" style={{ backgroundColor: ACCENT }}>L</span><span className="text-sm font-semibold uppercase">Dernière prise orale</span></div>
            <div className="flex flex-wrap gap-3 items-end"><div><span className="text-xs text-neutral-500">Heure</span><InputBox value={form.SAMPLER.sampler_l_time} onChange={(v) => updateField('SAMPLER', 'sampler_l_time', formatTimeInput(v))} placeholder="hh:mm" numeric width="w-24" /></div><ToggleGroup value={form.SAMPLER.sampler_l_choice} onChange={(v) => updateField('SAMPLER', 'sampler_l_choice', v)} options={REPAS_OPTIONS} /></div>
            <span className="text-xs text-neutral-500 uppercase">Nature</span>
            <MultiToggleGroup value={form.SAMPLER.sampler_l_nature} onChange={(v) => updateField('SAMPLER', 'sampler_l_nature', v)} options={PRISE_ORALE_NATURE_OPTIONS} />
            <InputBox value={form.SAMPLER.sampler_l} onChange={(v) => updateField('SAMPLER', 'sampler_l', v)} placeholder="Quoi ? / précision" width="w-full" />
          </div>

          <div className="bg-neutral-900 border border-neutral-800 rounded-lg p-4 flex flex-col gap-3">
            <div className="flex items-center gap-2"><span className="w-7 h-7 rounded flex items-center justify-center text-white text-sm font-bold" style={{ backgroundColor: ACCENT }}>E</span><span className="text-sm font-semibold uppercase">Événement</span></div>
            <MultiToggleGroup value={form.SAMPLER.sampler_e_choices} onChange={(v) => updateField('SAMPLER', 'sampler_e_choices', v)} options={EVENEMENT_OPTIONS} />
            <div><span className="text-xs text-neutral-500">Heure de l’événement</span><InputBox value={form.SAMPLER.sampler_e_time} onChange={(v) => updateField('SAMPLER', 'sampler_e_time', formatTimeInput(v))} placeholder="hh:mm" numeric width="w-24" /></div>
          ...
          <SamplerField letter="R" label="Facteurs de risque" value={form.SAMPLER.sampler_r} onChange={(v) => updateField('SAMPLER', 'sampler_r', v)} choiceOptions={RISQUE_OPTIONS} choiceValue={form.SAMPLER.sampler_r_choices} onChoiceChange={(v) => updateField('SAMPLER', 'sampler_r_choices', v)} choiceMulti />
          {autoRisks.length > 0 && <div className="bg-neutral-900 border border-neutral-800 rounded-lg p-3"><span className="text-xs text-neutral-500 uppercase">Déjà détecté ailleurs — sans double saisie</span><div className="flex flex-wrap gap-2 mt-2">{autoRisks.map((r) => <span key={r} className="text-xs px-2 py-1 rounded border border-neutral-700 text-neutral-300">✓ {r}</span>)}</div></div>}
        </>
      );
    }
    return (
