...
function computeMissingChecks(data) {
  ...
  if (!data.TYPE?.categorie) add('TYPE', 'Catégorie de victime non renseignée');

  const xMissing = missingLabels([
    ['traumatisme', data.X?.trauma],
    ['hémorragie', data.X?.hemorragie],
  ]);
  if (xMissing.length) add('X', `X non évalué : ${xMissing.join(', ')}`);
  ...
  if (aMissing.length) add('A', `A non évalué : ${aMissing.join(', ')}`);
  ...
  if (bMissing.length) add('B', `B incomplet : ${bMissing.join(', ')}`);
  ...
  if (cMissing.length) add('C', `C incomplet : ${cMissing.join(', ')}`);
  ...
  if (dMissing.length) add('D', `D incomplet : ${dMissing.join(', ')}`);
  ...
  if (eMissing.length) add('E', `E incomplet : ${eMissing.join(', ')}`);

  if (!data.BRULURE?.brulure) add('BRULURE', 'Évaluation de brûlure non renseignée');

  if (data.B.o2_active === 'oui') {
    if (!data.B.o2_interface) add('B', 'Interface O2 à renseigner');
    if (!data.B.o2_start_time) add('B', 'Heure de début O2 à renseigner');
    if (!data.B.o2_bottle_size) add('B', 'Taille de bouteille O2 à renseigner');
    if (!data.B.o2_pressure) add('B', 'Pression de la bouteille O2 à renseigner');
    if (!data.B.o2_debit) add('B', 'Débit O2 à renseigner');
    if (!data.B.spo2_o2) add('B', 'SpO2 sous O2 non renseignée');
  }
  ...
  if (fastPositif && !data.FAST.temps && !data.FAST.temps_choice) {
    add('FAST', "Heure d'apparition des signes FAST à renseigner");
  }
  ...
  if (fastStarted && (!data.FAST?.face || !data.FAST?.arm || !data.FAST?.speech)) {
    add('FAST', 'FAST commencé mais incomplet');
  }

  if (hasReal(data.D.neuro_signes) && !data.D.neuro_signes_depuis_heure) {
    add('D', "Heure d'apparition des signes neurologiques manquante");
  }
  if (hasReal(data.B.fr_signes) && !data.B.fr_signes_heure) {
    add('B', "Heure d'apparition des signes respiratoires manquante");
  }
  if (hasReal(data.C.signes) && !data.C.signes_heure) {
    add('C', "Heure d'apparition des signes circulatoires manquante");
  }

  if (data.D.pci === 'oui' && !data.D.pci_duree) {
    add('D', 'Durée de la PCI non renseignée');
  }
  if (data.D.pc_repete === 'oui' && !data.D.pc_nombre) {
    add('D', 'Nombre de pertes de connaissance non renseigné');
  }

  if (data.BRULURE.brulure === 'oui') {
    if (!data.BRULURE.brulure_etendue) {
      add('BRULURE', 'Étendue de la brûlure non renseignée');
    }
    const hasLoc =
      (data.BRULURE.brulure_loc_choices && data.BRULURE.brulure_loc_choices.length > 0) || !!data.BRULURE.brulure_loc;
    if (!hasLoc) {
      add('BRULURE', 'Localisation de la brûlure non renseignée');
    }
  }
    ...
    <div className="flex flex-col gap-5">
      {missingInfo.length > 0 && (
        ...
        >
          ...
          <ul className="flex flex-col gap-1 pl-1">
            {missingChecks.map(({ page, message }, i) => {
              const content = (
                <>
          ...
          {onNavigate && (
            <button
              type="button"
              onClick={() => onNavigate(missingChecks[0].page, true)}
...
export default function BilanVsav() {
  ...
  function goNext() {
    if (reviewMissingOnly) {
      ...
      if (nextMissing !== undefined) {
        ...
        const earlierMissing = missingIndexes.find((index) => index < step);
        if (earlierMissing !== undefined) {
          setStep(earlierMissing);
          return;
        }
  ...
  function renderStepContent() {
    ...
    if (s === 'A')
      return (
        <>
          <FieldCard label="Voies aériennes libres actuellement ?" filled={!!form.A.obstruction}>
            <ToggleGroup
